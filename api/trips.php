...
class TripsAPI {
    ...
    public function searchTrips($data) {
        ...
        // Enregistrer la recherche dans MongoDB
        global $mongodbService;
        if ($mongodbService->isEnabled()) {
            $mongodbService->storeSearchQuery(
                json_encode(['depart' => $depart, 'destination' => $destination, 'date' => $date]),
    ...
    public function participateTrip($data) {
        ...
        try {
            ...
            // Enregistrer l'activité dans MongoDB
            global $mongodbService;
            if ($mongodbService->isEnabled()) {
                $mongodbService->logActivity($user_id, 'trip_reservation', [
                    'trip_id' => $trip_id,
                    'depart' => $trip['depart'],
                    'destination' => $trip['destination'],
                    'prix' => $trip['prix']
                ...
                $mongodbService->storeNotification(
                    $trip['user_id'],
                    'new_reservation',
                    "Nouvelle réservation pour votre trajet {$trip['depart']} → {$trip['destination']}",
                    ['trip_id' => $trip_id, 'passenger_id' => $user_id]
                );
            }
